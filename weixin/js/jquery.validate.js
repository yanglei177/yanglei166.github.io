...
$.extend($.validator, {
	...
	defaults: {
		...
		unhighlight: function( element, errorClass, validClass ) {
			if ( element.type === "radio" ) {
				...
				$(element).parents(".verifydes").siblings(".des").css("visibility","hidden");
				$(element).parents(".verifydes").siblings(".des").show().end().siblings(".img_box,.imgarr").hide();				
			}
		}
	},
	...
	prototype: {
		...
		showLabel: function( element, message ) {
			...
			if ( label.length ) {
				...
				// create label
				label = $(element).parents(".verifydes").siblings(".des")				
					.html("<i class='imgsprite'></i>"+message || "");
				if ( this.settings.wrapper ) {
					// make sure the element is visible, even in IE
					// actually showing the wrapped element is handled elsewhere
					label = label.hide().show().wrap("<" + this.settings.wrapper + "/>").parent();
				...
				if ( !this.labelContainer.append(label).length ) {
					if ( this.settings.errorPlacement ) {
						...
						$(element).parents(".verifydes").siblings(label).css("visibility","visible");
						//$(element).parent().addClass("has-error");
					}
				}
			}
	...
	methods: {
		...
		isCardNo: function( value, element ) {
